...
export const BookingsPage = () => {
  ...
  useEffect(() => {
    const loadBookings = async () => {
      ...
      try {
        ...
        setCreateError(loadError instanceof Error ? loadError.message : "Unable to load bookings");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCreate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setBookings((current) => [...current, booking].sort((left, right) => left.startsAt.localeCompare(right.startsAt)));
      ...
      setCreateError(createError instanceof Error ? createError.message : "Unable to create booking");
    }
  };
  ...
  const handleSave = async (bookingId: string) => {
    try {
      const updatedBooking = await bookingService.update(bookingId, bookingEdits[bookingId]);
      setBookings((current) =>
  ...
  const handleDelete = async (bookingId: string) => {
    try {
      ...
      setBookingErrors((current) => ({
        ...current,
        [bookingId]: deleteError instanceof Error ? deleteError.message : "Unable to delete booking"
      }));
    }
  };
        ...
        <div className="space-y-4">
          {bookings.length ? (
            bookings.map((booking) => {
              ...
              return (
                <article className="rounded-3xl bg-white p-6 shadow-sm" key={booking._id}>
